...
export function LeadFormDialog({ lead, onClose, onSuccess }: { lead?: Lead; onClose: () => void; onSuccess: (message: string) => void }) {
  ...
  return <Dialog title={lead ? 'Editar lead' : 'Criar lead'} onClose={onClose}><LeadForm lead={lead} submitting={saving} error={error} onSubmit={submit} onCancel={onClose} /></Dialog>
}
