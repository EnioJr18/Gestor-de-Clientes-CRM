...
export function InteractionsEvolutionChart({ items }: { items: DashboardInteractionEvolutionItem[] }) {
  ...
  return <article className="lead-card"><h2 className="text-lg font-semibold text-strong">Evolucao de interacoes</h2><p className="mt-1 text-sm text-muted">Contatos registrados em cada mes do periodo.</p><div className="mt-5 h-72"><Bar aria-label="Grafico de evolucao de interacoes" data={{ labels: items.map((item) => item.label), datasets: [{ label: 'Interacoes registradas', data: items.map((item) => item.count), backgroundColor: '#8b6cff', hoverBackgroundColor: '#a896ff', borderRadius: 8 }] }} options={{ responsive: true, maintainAspectRatio: false, animation: reduced ? false : { duration: 800, easing: 'easeOutQuart' }, scales: { y: { beginAtZero: true, ticks: { precision: 0, color: '#b5c0d7' }, grid: { color: '#2a3655' } }, x: { ticks: { color: '#b5c0d7' }, grid: { display: false } } }, plugins: { tooltip: { backgroundColor: '#121a30', titleColor: '#f2f5ff', bodyColor: '#f2f5ff' } } }} /></div><dl className="sr-only">{items.map((item) => <div key={item.month}><dt>{item.label}</dt><dd>{item.count} interacoes registradas</dd></div>)}</dl></article>
}
